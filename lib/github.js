...
export async function writeFile(repoString, path, content, sha, message, branch = 'master') {
  const { owner, repo } = parseRepo(repoString);
  const params = { owner, repo, path, message, content: Buffer.from(content).toString('base64'), branch };
  if (sha) params.sha = sha; // Only include sha when updating (omit for new file creation)
  const res = await getOctokit().rest.repos.createOrUpdateFileContents(params);
