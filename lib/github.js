...
export async function readFile(repoString, path, branch = 'main') {
...
export async function writeFile(repoString, path, content, sha, message, branch = 'main') {
  const { owner, repo } = parseRepo(repoString);
...
export async function listFiles(repoString, path, branch = 'main') {
  const { owner, repo } = parseRepo(repoString);
  const res = await getOctokit().rest.repos.getContent({
    owner,
    repo,
...
export async function createBranch(repoString, branchName) {
  const { owner, repo } = parseRepo(repoString);
  const main = await getOctokit().rest.git.getRef({ owner, repo, ref: 'heads/main' });
...
export async function mergeBranch(repoString, branchName, message) {
  const { owner, repo } = parseRepo(repoString);
